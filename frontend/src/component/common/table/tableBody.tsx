    ...
    <tbody>
      {data.map((row, index) => {
        ...
        return (
          <tr
            key={`row-${index}`}
            className={`${rowSizeClass} ${stripedClass} ${rowClassName ? rowClassName(row) : ""}`}
            onClick={() => {
          ...
          >
            {columns.map((col) => {
              ...
              return (
                <td
                  key={col.key}
                  className={`px-3 py-2 border-b border-gray-200 truncate ${alignClass}`}
                >
      ...
      {emptyCount > 0 &&
        Array.from({ length: emptyCount }).map((_, i) => (
          <tr key={`empty-${i}`} className={rowSizeClass}>
            {columns.map((col) => (
              <td key={col.key} className="px-3 py-2 bg-white">
                &nbsp;
              </td>
            ))}
          </tr>
