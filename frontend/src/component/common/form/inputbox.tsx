...
const InputBox = ({
  ...
  const sizeStyles = {
    ...
    md: {
      wrapper: "h-10 text-xs px-3 gap-2",
      input: "text-xs",
  ...
  const borderColor = (() => {
    ...
    if (focused) return "border-gray-300 dark:border-neutral-600";
    return "border hover:border-gray-300 dark:border-neutral-700 dark:hover:border-neutral-600";
  })();

  const widthStyle = full ? "w-full" : width ? undefined : "inline-flex";

  const baseBg = className.includes("bg-") ? "" : "bg-white dark:bg-neutral-800";
