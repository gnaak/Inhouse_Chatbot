import { REQUEST_STATUSMAP } from "@/mapping/admin/map";
...
const DetailDirectory = ({ targetInfo }) => {
  ...
  const waitingList = targetInfo?.directory_list?.filter((r) => r.status === "waiting") ?? [];
  const approvedList = targetInfo?.directory_list?.filter(
    (r) => r.status === "approved" && r.directory_id !== 1,
  ) ?? [];
