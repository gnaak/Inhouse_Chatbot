...
class AuthToken:
    ...
    domain = settings.prod_cookie_domain if env == "prod" else None
    ...
    def __init__(self):
        ...
        self.domain = settings.prod_cookie_domain if self.env == "prod" else None
    ...
    async def get_token_info(self, request, auth_type: str):
        ...
        서명된 HttpOnly 쿠키만 신뢰한다. Authorization 헤더 경로는 두지 않는다 —
        서명 검증 없는 토큰을 받는 순간 아무나 user_id 를 지정할 수 있다.
        """
    ...
    # TODO: NATIVE
    async def create_jwt_for_native(self, user_id, user_name, created_at, type):
        now_kr = now_kst()
        ...
        return access_token, refresh_token, encoded_info
